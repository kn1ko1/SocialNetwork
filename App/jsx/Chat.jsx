...
export function Chat({ socket }) {
  ...
  useEffect(() => {
    ...
    const fetchUserAndGroupData = async () => {
      try {
        ...
        promises.push(
          fetch(
            `http://localhost:8080/api/users/${currentUserId}/followedUsers`
          )
        );
        promises.push(
          fetch(
            `http://localhost:8080/api/users/${currentUserId}/followerUsers`
          )
        );
        promises.push(
          fetch(`http://localhost:8080/api/users/${currentUserId}/groups`)
        );
        ...
        if (usersIFollowData != null && usersFollowMeData != null) {
          uniqueUsers = Array.from(
            new Set([...usersIFollowData, ...usersFollowMeData])
          );
  ...
  const handleUserClick = async (user) => {
    ...
    const messagesResponse = await fetch(
      `http://localhost:8080/api/users/${currentUserId}/messages/${user.userId}`
    );
    if (!messagesResponse.ok) {
      throw new Error(
        `Failed to fetch messages between user ${currentUserId} and user ${user.userId}`
      );
    }
  ...
  const handleGroupClick = async (group) => {
    ...
    const messagesResponse = await fetch(
      `http://localhost:8080/api/groups/${group.groupId}/messages`
    );
    if (!messagesResponse.ok) {
  ...
  socket.onmessage = function (e) {
    ...
    let chatHistory = document.getElementById("chatHistory");
    if (
      (data.code == 1 && selectedGroup.groupId == message.targetId) ||
      (data.code == 2 && selectedUser.username == message.senderUsername)
    ) {
      const messageCard = createMessageCard(message);
      if (chatHistory.childNodes.length > 0) {
        chatHistory.prepend(messageCard); // Add to the start
  ...
  const messageStyle = {
    ...
    textAlign: "center",
    backgroundColor: "linear-gradient(to bottom, #c7ddef, #ffffff)", // Light gray background
  ...
  return (
    <div className="container-fluid">
      <div className="row">
        {/* Users and Groups Column */}
        <div
          className="col-md-4"
          style={{ ...opaqueStyle, height: "100vh", overflowY: "auto" }}
        >
          <div style={chatStyle}>
        ...
        >
          ...
          >
            ...
            {selectedGroup && <h3>Chat in {selectedGroup.title}</h3>}
          </h3>
          ...
          <form id="chatbox" onSubmit={handleSubmit}>
            ...
            >
              ...
              {isEmojiPickerVisible && (
                ...
                >
                  <button type="button" onClick={() => handleEmojiSelect("😊")}>
                    😊
                  </button>
                  <button type="button" onClick={() => handleEmojiSelect("😂")}>
                    😂
                  </button>
                  <button type="button" onClick={() => handleEmojiSelect("❤️")}>
                    ❤️
                  </button>
                  <button type="button" onClick={() => handleEmojiSelect("👍")}>
                  👍
