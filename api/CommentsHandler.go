...
func (h *CommentsHandler) post(w http.ResponseWriter, r *http.Request) {
	// Read the JSON body of the request OR parse form data to get the comment
	// (we are gonna do both, using headers correctly we can separate UI logic from API logic)
	//
	// Again, ommitted here for sake of example. We just assume this is what user is trying to comment:
	comment := models.Comment{CommentId: 1, Body: "Example"}
	...
	// 	return
	// }
	// Handler uses its Repo instance to add the comment to the DB
	...
	result, err := h.Repo.CreateComment(comment)
	if err != nil {
		log.Println(err.Error())
...
func (h *CommentsHandler) get(w http.ResponseWriter, r *http.Request) {
	// Not Implemented - would be h.Repo.GetAllComments() ... you get the idea
